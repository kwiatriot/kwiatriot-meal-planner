...
function RecipeCard({ recipe }: { recipe: RecipeRow }) {
  ...
  return (
    <Link
      ...
      className="block rounded-xl border border-gray-200 bg-white p-4 shadow-sm hover:shadow-md hover:border-blue-300 transition-all dark:border-gray-800 dark:bg-gray-900 dark:hover:border-blue-700"
    >
      <h3 className="font-semibold text-gray-900 leading-snug dark:text-gray-100">{recipe.name}</h3>
      {recipe.description && (
        <p className="mt-1 text-sm text-gray-500 line-clamp-2 dark:text-gray-400">{recipe.description}</p>
      )}
...
export default async function MealSelectorPage() {
  ...
  if (error) {
    return (
      <main className="p-6">
        <p className="text-red-600 dark:text-red-400">Failed to load recipes: {error.message}</p>
      </main>
    )
  }
      ...
      <div className="mx-auto max-w-5xl">
        ...
        {(recipes ?? []).length === 0 ? (
          <div className="rounded-xl border border-dashed border-gray-300 p-12 text-center dark:border-gray-700">
            ...
            <Link href="/recipes/new" className="mt-2 inline-block text-blue-600 hover:underline text-sm dark:text-blue-400">
          ...
          <div className="space-y-8">
            {TYPE_ORDER.filter((t) => grouped[t]?.length > 0).map((type) => (
              <section key={type}>
                <h2 className="mb-3 text-lg font-semibold text-gray-700 dark:text-gray-300">{TYPE_LABELS[type]}</h2>
